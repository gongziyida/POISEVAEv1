...
class GibbsSampler():
    ...
    def value_calc(self,z, g11, g22, lambda_1, lambda_2):
        var = self.var_calc(z, g22, lambda_2)
        mean = self.mean_calc(z, var, g11, lambda_1)
        out = mean + torch.sqrt(var.float()) * torch.randn_like(var)
        return out

    def sample(self, g11, g22, z=None, lambda1s=None, lambda2s=None, n_iterations=1, n_samples=1, 
               batch_size=None):
        """
        g11, g22: 
            Diagonal blocks of the metric tensor
        ...
        g11T, g22T = torch.transpose(g11,0,1), torch.transpose(g22,0,1)
        # TODO: make it generic for > 2 latent spaces 
        for i in range(n_iterations):
            z[0][0] = self.value_calc(z[1][0], g11T, g22T, lambda1s[0], lambda2s[0]) 
            z[1][0] = self.value_calc(z[0][0], g11, g22, lambda1s[1], lambda2s[1])
        
        for i in range(n_samples - 1):
            z[0].append(self.value_calc(z[1][-1], g11T, g22T, lambda1s[0], lambda2s[0]))
            z[1].append(self.value_calc(z[0][-1], g11, g22, lambda1s[1], lambda2s[1]))
        z = [torch.stack(z[i], dim=1) for i in range(len(z))]
