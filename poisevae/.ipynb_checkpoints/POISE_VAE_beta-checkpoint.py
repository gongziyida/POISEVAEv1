...
class POISEVAE(nn.Module):
    ...
    def __init__(self, encoders, decoders, loss_funcs, latent_dims=None, batch=True, n_samples=1,
                 device=_device):
        """
        Parameters
        ----------
        ...
        n_samples: int, default 1
            How many samples to calculate the gradient for KL divergence
    ...
    def _init_gibbs(self, g22, mu, var, n_iterations=5000):
        ...
        self.z_priors = [z[:, -1].detach().squeeze(1) for z in z_priors]
        self.z_posteriors = [z[:, -1].detach().squeeze(1) for z in z_posteriors]
    ...
    def _sampling(self, g22, mu, var, n_iterations=5):
        ...
        z_gibbs_priors = self.gibbs.sample(self.g11, g22, z=z_priors, 
                                           n_iterations=n_iterations, n_samples=1)

        z_gibbs_posteriors = self.gibbs.sample(self.g11, g22, lambda1s=mu, lambda2s=var, z=z_posteriors, 
                                               n_iterations=n_iterations, n_samples=self.n_samples)

        # For calculating the loss and future use
        # [:, -1] select the last sample
        self.z_priors = [z[:, -1].detach().squeeze(1) for z in z_gibbs_priors]
        self.z_posteriors = [z[:, -1].detach().squeeze(1) for z in z_gibbs_posteriors]
    ...
    def forward(self, x):
        ...
        """
        # for the last batch whose size is often different
        batch_size = x[0].shape[0] if self.batch else 1
        if batch_size < self._batch_size and self.flag_initialize == 0: 
            for i in range(self.M):
                self.z_priors[i] = self.z_priors[i][:batch_size]
                self.z_posteriors[i] = self.z_priors[i][:batch_size]
            self._batch_size = batch_size
        ...
        # KL loss
        kls = self.kl_div.calc(G, z_gibbs_priors, z_gibbs_posteriors, mu, var)
