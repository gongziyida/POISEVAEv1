import torch
import torch.nn as nn
class MaskedConv2d(nn.ConvTranspose2d):
	def __init__(self, mask_type, *args, **kwargs):
		self.mask_type = mask_type
		assert mask_type in ['A', 'B'], "Unknown Mask Type"
		super(MaskedConv2d, self).__init__(*args, **kwargs)
		self.register_buffer('mask', self.weight.data.clone())

		_, depth, height, width = self.weight.size()
		self.mask.fill_(1)
		if mask_type =='A':
			self.mask[:,:,height//2,width//2:] = 0
			self.mask[:,:,height//2+1:,:] = 0
		else:
			self.mask[:,:,height//2,width//2+1:] = 0
			self.mask[:,:,height//2+1:,:] = 0


	def forward(self, x):
		self.weight.data*=self.mask
		return super(MaskedConv2d, self).forward(x)

class EncMNIST(nn.Module):
    def __init__(self, latent_dim):
        super(EncMNIST, self).__init__()
        self.latent_dim = latent_dim
        
        n_channels = (1, 32, 64, 128, 256)
        kernels = (4, 4, 4, 4)
        strides = (2, 2, 2, 2)
        paddings = (1, 1, 1, 1)
        li = []
        for i, (n, k, s, p) in enumerate(zip(n_channels[1:], kernels, strides, paddings), 1):
            li += [nn.Conv2d(n_channels[i-1], n, kernel_size=k, stride=s, padding=p), 
                nn.ReLU(inplace=True)]
        self.enc = nn.Sequential(*li)
        self.enc_mu = nn.Conv2d(in_channels=256, out_channels=latent_dim, 
                                kernel_size=1, stride=1, padding=0)
        self.enc_var = nn.Conv2d(in_channels=256, out_channels=latent_dim, 
                                 kernel_size=1, stride=1, padding=0)
        
    def forward(self, x):
        x = self.enc(x)
        # Be careful not to squeeze the batch dimension if batch size = 1
        mu = self.enc_mu(x).squeeze(3).squeeze(2)
        log_var = self.enc_var(x).squeeze(3).squeeze(2)
        return mu, log_var

class DecMNIST(nn.Module):
    def __init__(self, latent_dim, img_dim):
        super(DecMNIST, self).__init__()  
        self.latent_dim = latent_dim
        self.img_dim = img_dim
        
        n_channels = (latent_dim, 256, 128, 64, 28, 1)
        kernels = (2, 4, 3, 4, 4)
        strides = (1, 2, 2, 2, 2)
        paddings = (0, 1, 1, 1, 1)
        li = []
        for i, (n, k, s, p) in enumerate(zip(n_channels[1:], kernels, strides, paddings), 1):
            if i==0:
                li += [MaskedConv2d('A',n_channels[i-1], n, kernel_size=k, stride=s, padding=p), 
                       nn.ReLU(inplace=True)]
            else:
                li += [MaskedConv2d('B',n_channels[i-1], n, kernel_size=k, stride=s, padding=p), 
                        nn.ReLU(inplace=True)]
        li[-1] = nn.Sigmoid()
        
        self.dec = nn.Sequential(*li)

        self.z2x = nn.Sequential(nn.Linear(n_channels, 8), nn.ReLU(inplace=True), nn.Linear(8, 1))
        
    def forward(self, z):
        pixel = self.z2x(z)
        x = torch.zeros(z.shape[0], *self.img_dim).to(z.device)
        x[0, 0] = 
        return self.dec(x), torch.tensor(0.75).to(z.device)
    
class EncSVHN(nn.Module):
    def __init__(self, latent_dim):
        super(EncSVHN, self).__init__()
        self.latent_dim = latent_dim
        
        n_channels = (3, 32, 64, 128, 256)
        kernels = (4, 4, 4, 4)
        strides = (2, 2, 2, 2)
        paddings = (1, 1, 1, 1)
        li = []
        for i, (n, k, s, p) in enumerate(zip(n_channels[1:], kernels, strides, paddings), 1):
            li += [nn.Conv2d(n_channels[i-1], n, kernel_size=k, stride=s, padding=p), 
                   nn.ReLU(inplace=True)]
            
        self.enc = nn.Sequential(*li)
        self.enc_mu = nn.Conv2d(in_channels=256, out_channels=latent_dim, 
                                kernel_size=2, stride=1, padding=0)
        self.enc_var = nn.Conv2d(in_channels=256, out_channels=latent_dim, 
                                 kernel_size=2, stride=1, padding=0)
        
    def forward(self, x):
        x = self.enc(x)
        # Be careful not to squeeze the batch dimension if batch size = 1
        mu = self.enc_mu(x).squeeze(3).squeeze(2)
        log_var = self.enc_var(x).squeeze(3).squeeze(2)
        return mu, log_var
    
class DecSVHN(nn.Module):
    def __init__(self, latent_dim):
        super(DecSVHN, self).__init__()  
        self.latent_dim = latent_dim
        
        n_channels = (latent_dim, 256, 128, 64, 32, 3)
        kernels = (2, 4, 4, 4, 4)
        strides = (1, 2, 2, 2, 2)
        paddings = (0, 1, 1, 1, 1)
        li = []
        for i, (n, k, s, p) in enumerate(zip(n_channels[1:], kernels, strides, paddings), 1):
            li += [nn.ConvTranspose2d(n_channels[i-1], n, kernel_size=k, stride=s, padding=p), 
                   nn.ReLU(inplace=True)]
        li[-1] = nn.Sigmoid()
        
        self.dec = nn.Sequential(*li)
        
    def forward(self, z):
        return self.dec(z), torch.tensor(0.75).to(z.device)
